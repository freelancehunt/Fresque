...
class Worker
{
     ...
     */
    public function __construct(mixed $queues)
    {
        if (!is_array($queues)) {
            $queues = [$queues];
        }
        ...
        while (true) {
            ...
            if (!$this->paused) {
                try {
                    ...
                    $this->log(['message' => 'Redis exception caught: ' . $e->getMessage(), 'data' => ['type' => 'fail', 'log' => $e->getMessage(), 'time' => time()]], self::LOG_TYPE_ALERT);
                }
            }
            ...
            if (!$job) {
                ...
                $this->log(['message' => 'Sleeping for ' . $interval, 'data' => ['type' => 'sleep', 'second' => $interval]], self::LOG_TYPE_DEBUG);
                if ($this->paused) {
            ...
            if ($this->child === 0 || $this->child === false) {
                ...
                $this->log(['message' => $status, 'data' => ['type' => 'process', 'worker' => $workerName, 'job_id' => $job->payload['id']]], self::LOG_TYPE_INFO);
                $this->perform($job);
                if ($this->child === 0) {
                    exit(0);
                }
            ...
            if ($this->child > 0) {
                ...
                $this->updateProcLine($status);
                $this->log(['message' => $status, 'data' => ['type' => 'fork', 'worker' => $workerName, 'job_id' => $job->payload['id']]], self::LOG_TYPE_DEBUG);
    ...
    public function perform(Resque_Job $job)
    {
        ...
        try {
            ...
            $job->perform();
            $this->log(['message' => 'done ID:' . $job->payload['id'], 'data' => ['type' => 'done', 'job_id' => $job->payload['id'], 'time' => round(microtime(true) - $startTime, 3) * 1000]], self::LOG_TYPE_INFO);
        } catch (Exception $e) {
            $this->log(['message' => $job . ' failed: ' . $e->getMessage(), 'data' => ['type' => 'fail', 'log' => $e->getMessage(), 'job_id' => $job->payload['id'], 'time' => round(microtime(true) - $startTime, 3) * 1000]], self::LOG_TYPE_ERROR);
            $job->fail($e);
    ...
    {
        $this->log(['message' => 'Starting worker ' . $this, 'data' => ['type' => 'start', 'worker' => (string) $this]], self::LOG_TYPE_INFO);
    ...
    protected function registerSigHandlers()
    {
        if (!function_exists('pcntl_signal')) {
            $this->log(['message' => 'Signals handling is unsupported', 'data' => ['type' => 'signal']], self::LOG_TYPE_WARNING);
    ...
    {
        $this->log(['message' => 'USR2 received; pausing job processing', 'data' => ['type' => 'pause']], self::LOG_TYPE_INFO);
        $this->paused = true;
    }
    ...
    {
        $this->log(['message' => 'CONT received; resuming job processing', 'data' => ['type' => 'resume']], self::LOG_TYPE_INFO);
        $this->paused = false;
    }
    ...
    {
        $this->log(['message' => 'SIGPIPE received; attempting to reconnect', 'data' => ['type' => 'reconnect']], self::LOG_TYPE_INFO);
    ...
    {
        if (!$this->child) {
            $this->log(['message' => 'No child to kill.', 'data' => ['type' => 'kill', 'child' => null]], self::LOG_TYPE_DEBUG);

            return;
        }

        $this->log(['message' => 'Killing child at ' . $this->child, 'data' => ['type' => 'kill', 'child' => $this->child]], self::LOG_TYPE_DEBUG);
        if (exec('ps -o pid,state -p ' . $this->child, $output, $returnCode) && $returnCode != 1) {
            $this->log(['message' => 'Killing child at ' . $this->child, 'data' => ['type' => 'kill', 'child' => $this->child]], self::LOG_TYPE_DEBUG);
            posix_kill($this->child, SIGKILL);
            $this->child = null;
        } else {
            $this->log(['message' => 'Child ' . $this->child . ' not found, restarting.', 'data' => ['type' => 'kill', 'child' => $this->child]], self::LOG_TYPE_ERROR);
            $this->shutdown();
        }
    }
    ...
    public function pruneDeadWorkers()
    {
        ...
        foreach ($workers as $worker) {
            if (is_object($worker)) {
                ...
                $this->log(['message' => 'Pruning dead worker: ' . (string) $worker, 'data' => ['type' => 'prune']], self::LOG_TYPE_DEBUG);
    ...
    public function log($message, $code = self::LOG_TYPE_INFO)
    {
        ...
        if (is_array($message)) {
            $extra   = is_array($message['data']) ? $message['data'] : [$message['data']];
